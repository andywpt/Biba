...
public class FirebaseDataEncoder {
  ...
  open var passthroughTypeResolver: StructureCodingPassthroughTypeResolver.Type = NoPassthroughTypes
    .self
...
private struct _JSONKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
  ...
  fileprivate init(
    referencing encoder: __JSONEncoder, codingPath: [CodingKey],
  ...
  private func _converted(_ key: CodingKey) -> CodingKey {
    ...
    case .useDefaultKeys:
      return key
    case .convertToSnakeCase:
      let newKeyString = FirebaseDataEncoder.KeyEncodingStrategy._convertToSnakeCase(
        key.stringValue)
  ...
  public mutating func superEncoder() -> Encoder {
    return __JSONReferencingEncoder(
      referencing: self.encoder, key: _JSONKey.super, convertedKey: _converted(_JSONKey.super),
      wrapping: self.container)
  }

  public mutating func superEncoder(forKey key: Key) -> Encoder {
    return __JSONReferencingEncoder(
      referencing: self.encoder, key: key, convertedKey: _converted(key), wrapping: self.container)
  }
}
...
private struct _JSONUnkeyedEncodingContainer: UnkeyedEncodingContainer {
  ...
  /// Initializes `self` with the given references.
  fileprivate init(
    referencing encoder: __JSONEncoder, codingPath: [CodingKey], wrapping container: NSMutableArray
  ) {
    self.encoder = encoder
    self.codingPath = codingPath
    self.container = container
...
extension __JSONEncoder {
  ...
  fileprivate func box_(_ value: Encodable) throws -> NSObject? {
    ...
    if type == Date.self || type == NSDate.self {
      ...
      self.options.passthroughTypeResolver.isPassthroughType(value)
    {
      return object
    }
...
private class __JSONReferencingEncoder: __JSONEncoder {
  ...
  fileprivate init(
    referencing encoder: __JSONEncoder, at index: Int, wrapping array: NSMutableArray
  ) {
    self.encoder = encoder
...
public class FirebaseDataDecoder {
  ...
  open func decode<T: Decodable>(_ type: T.Type, from structure: Any) throws -> T {
    ...
    guard let value = try decoder.unbox(structure, as: type) else {
      throw Swift.DecodingError.valueNotFound(
        type,
    ...
    if let rcValue = self.storage.topContainer as? FirebaseRemoteConfigValueDecoding {
      guard let top = rcValue.dictionaryValue() else {
        throw DecodingError._typeMismatch(
          at: self.codingPath, expectation: [String: Any].self,
          reality: rcValue)
      }
      topContainer = top
    } else {
    ...
    case .custom(let converter):
      self.container = Dictionary(
        container.map {
          key, value in
          (
            converter(decoder.codingPath + [_JSONKey(stringValue: key, intValue: nil)]).stringValue,
            value
          )
        }, uniquingKeysWith: { (first, _) in first })
    }
  ...
  private func _errorDescription(of key: CodingKey) -> String {
    ...
    case .convertFromSnakeCase:
      ...
      if converted == original {
        ...
        return
          "\(key) (\"\(original)\"), with divergent representation \(roundtrip), converted to \(converted)"
      }
    default:
  ...
  public mutating func decodeNil() throws -> Bool {
    guard !self.isAtEnd else {
      throw DecodingError.valueNotFound(
        Any?.self,
        DecodingError.Context(
          codingPath: self.decoder.codingPath + [_JSONKey(index: self.currentIndex)],
...
extension __JSONDecoder {
  ...
  fileprivate func getNumber(_ value: Any, as type: Any.Type) throws -> NSNumber {
    ...
    guard let number = val as? NSNumber, number !== kCFBooleanTrue, number !== kCFBooleanFalse
    else {
  ...
  fileprivate func unbox(_ value: Any, as type: Float.Type) throws -> Float? {
    ...
    if let number = val as? NSNumber, number !== kCFBooleanTrue, number !== kCFBooleanFalse {
      ...
      case .convertFromString(let posInfString, let negInfString, let nanString) = self.options
  ...
  fileprivate func unbox<T>(_ value: Any, as type: _JSONStringDictionaryDecodableMarker.Type) throws
    -> T?
  {
    guard !(value is NSNull) else { return nil }
...
extension EncodingError {
  ...
  fileprivate static func _invalidFloatingPointValue<T: FloatingPoint>(
    ...
    let debugDescription =
      "Unable to encode \(valueDescription) directly in JSON. Use JSONEncoder.NonConformingFloatEncodingStrategy.convertToString to specify how the value should be encoded."
    return .invalidValue(
